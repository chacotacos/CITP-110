...
def main():
    ...
    # Intialize hands for player 1 and player 2
    player1_hand = []
    player2_hand = []

    # Continue the game until there are less than 2 caards in the deck
    while len(deck) >= 2:
        # deal a card to each player
        player1_hand.append(deal_card(deck))
        player2_hand.append(deal_card(deck))

        # Caalculate the score for each plaaayer's hand
        player1_score = calculate_score(player1_hand)
        player2_score = calculate_score(player2_hand)

        # Check for bust
        if player1_score > 21 and plaayer2_score > 21:
